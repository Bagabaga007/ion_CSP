...
class read_mlp_density:
    def __init__(self, target_folder:str):
        # 获取脚本的当前目录
        self.base_dir = os.path.dirname(target_folder)
        os.chdir(self.base_dir)
        # 寻找同一目录下的optimized文件夹
        self.target_combo = target_folder.split('/')[-2]
        ...
        print(f'Processing MLP CONTCARs in {target_folder}')
        logging.info(f'Processing MLP CONTCARs in {target_folder}')
    ...
    def read_density_and_sort(self, n_screen: int=10, N5_screen: bool=True, count_N5: int=2, detail_log: bool=True):
        """
    ...
    def phonopy_processing_max_density(self, specific_directory:str=None):
        ...
        # 移除最后复制多出来的POSCAR、BPOSCAR以及phonopy_symcells.yaml、nohup.out
        ...
        os.remove(f'{self.phonopy_dir}/nohup.out')
        shutil.copytree(self.primitive_cell_dir, f'{self.base_dir}/3_for_vasp_opt')
...
def main(work_dir, config):
    ...
    target_folder = os.path.join(work_dir, '2_mlp_optimized')
    result = read_mlp_density(target_folder)
    # 读取密度数据以及N5环是否独立存在，并将前n个最大密度的文件保存到max_density文件夹
    result.read_density_and_sort(n_screen=config['read_mlp_density']['n_screen'], 
                                 N5_screen=config['read_mlp_density']['N5_screen'], 
                                 count_N5=config['read_mlp_density']['count_N5'])
