...
class SMILES_processing:
    def __init__(self, work_dir: str, csv_file: str, target_file_type: str):
        """
        args:
        ...
        if target_file_type not in expeceted_file_types:
            raise KeyError(
                f"Not supported file type, the expected file type is one of {expeceted_file_types}"
            )
    ...
    def screen(self, charge_screen: int=0, group_screen: str = None, group_name: str = None):
        '''
        ...
        screened = self.df
        if group_screen:
            screened = screened[screened['SMILES'].str.contains(group_screen, regex=False)]
        if charge_screen:
...
def main(work_dir, config):
    ...
    result = SMILES_processing(work_dir, csv_file=config['convert_SMILES']['csv_file'], 
                               target_file_type=config['convert_SMILES']['target_file_type'])
    # 根据电荷进行分组创建文件夹并将SMILES码转换为对应的结构文件
    result.charge_group()
    if config['convert_SMILES']['screen']:
        # 根据提供的官能团和电荷进行筛选, 在本数据集中硝基的SMILES码为[N+](=O)[O-]
        result.screen(charge_screen=config['convert_SMILES']['charge_screen'], 
