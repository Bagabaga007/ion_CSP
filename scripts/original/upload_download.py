...
@log_and_time
def main(work_dir, up_down, config):
    batch_config = {"upload_prefixes": ["CONTCAR_", "OUTCAR_"]}
    folder_name = os.path.normpath(os.path.abspath(work_dir)).split(os.sep)[-1]

    job = SSHBatchJob(work_dir=work_dir, machine_json=config["upload_download"]["machine_json"], machine_type="ssh_direct")
    if up_down == "up":
        script = "steps_opt_monitor.sh"
        command = f"./steps_opt_monitor.sh {folder_name}"
        job.prepare_and_submit(command=command, forward_common_files=[script], batch_config=batch_config)
    elif up_down == "down":
        try:
        ...
        raise ValueError(
            'Not an avalible "up_down" parameter, which should be "up" or "down"'
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("up_down", type=str, help="'up' for upload, 'down' for download")
    ...
    try:
        ...
        raise
    # 调用主函数
    main(args.work_dir, args.up_down, config)
