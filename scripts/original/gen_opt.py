...
class CrystalGenerator:
    def __init__(self, work_dir: str, ion_numbers: List[int], species: List[str]):
        ...
        self.run_opt_file = os.path.join(script_dir, 'YZ_run_opt.py')
        self.model_file = os.path.join(script_dir, '../model/model.ckpt-4000000.pt')
        ...
        # 定义要检查的文件列表
        self.required_files = [self.run_opt_file, self.model_file]
        # 遍历文件列表，检查所需的每个文件是否存在
        for file in self.required_files:
            # 准备dpdispatcher运行所需的文件，将其复制到primitive_cell文件夹中
            if file != self.model_file:  # 由于势函数文件过大，不进行反复复制，防止复制过程出错
                shutil.copy(file, self.primitive_cell_dir)
        logging.info('The necessary files are fully prepared.')

    def _sequentially_read_files(self, dir: str, prefix_name: str='POSCAR_'):
        """
        Private method: 
    ...
    def generate_structures(self, num_per_group: int = 100, space_groups_limit: int = 230):
        """
    ...
    def prepare_and_submit(self, machine_json: str, resources_json: str, python_path: str='python', task_alloc: int=1):
        """
        ...
        machine = Machine.load_from_json(machine_json)
        resources = Resources.load_from_json(resources_json)
        ...
        if machine_inform['context_type'] == 'SSHContext':
            # 如果调用远程服务器，则创建二级目录
            parent = 'data/'
            forward_common_files = [self.model_file]
        ...
        for pop in range(task_alloc):
            ...
            command = f'{python_path} YZ_run_opt.py'
...
def main(work_dir, gen_opt, config):
    generator = CrystalGenerator(work_dir, ion_numbers=config['gen_opt']['ion_numbers'], species=config['gen_opt']['species'])
    if gen_opt == 'gen':
        ...
        generator.prepare_and_submit(machine_json=config['gen_opt']['machine_json'], 
                                     resources_json=config['gen_opt']['resources_json'],
                                     python_path=config['gen_opt']['python_path'],
                                     task_alloc=config['gen_opt']['task_alloc'])
    elif gen_opt == 'gen_opt':
        # 根据提供的离子与对应的配比，使用 pyxtal 基于晶体空间群进行离子晶体结构的随机生成。
        generator.generate_structures(num_per_group=config['gen_opt']['num_per_group'], 
                                      space_groups_limit=config['gen_opt']['space_groups_limit'])
        # 使用 phonopy 生成对称化的原胞另存于 primitive_cell 文件夹中，降低后续优化的复杂性，同时检查原子数以防止 pyxtal 生成双倍比例的超胞。
        ...
        raise ValueError('Not an avalible "gen_opt" parameter, which should be "gen", "opt" or "gen_opt"')
