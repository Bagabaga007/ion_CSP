...
class CrystalGenerator:
    def __init__(self, base_dir, work_dir, species:list, ion_numbers:list, conventional=False):
        """
        ...
        for ion, number in zip(species, ion_numbers):
            species_dir = os.path.join(work_dir, ion)
        ...
        self.POSCAR_dir = os.path.join(work_dir, 'POSCAR_Files')
        os.makedirs(self.POSCAR_dir, exist_ok=True)  # 如果目录不存在，则创建POSCAR_Files文件夹
        self.primitive_cell_dir = os.path.join(work_dir, 'primitive_cell')
        os.makedirs(self.primitive_cell_dir, exist_ok=True)
        # 根据conventional参数确定是否要保留conventional_cell结构文件
        self.conventional = conventional
        if self.conventional:
            self.conventional_cell_dir = os.path.join(work_dir, 'conventional_cell')
    ...
    def prepare_and_submit(self, machine_file, resources_file, python_path, task_alloc=4):
        """
        ...
        for index, _ in primitive_cell_file_index_pairs:
            ...
            alloc_jobs[alloc_index].append(index)
        task_list = []
        ...
        optimized_dir = os.path.join(self.work_dir, 'optimized')
...
def main():
    base_dir = os.path.dirname(__file__)
    folder_dir = os.path.join(base_dir, '2_0_batch_gen_results')
